...
const Playground: React.FC<PlaygroundProps> = ({
  ...
  let [userCode, setUserCode] = useState<string>(
    problem.starterCode[settings.lang]
  );
  ...
  function executeWithTimeout(callback: any, timeout = 1000) {
        ...
        [
          `
			onmessage = function(e) {
			  ...
			  postMessage({ result: "Task completed" });
			}
		  `,
        ],
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (typeof handler === "function") {
        try {
          const res = await executeWithTimeout(cb.toString());
  ...
  useEffect(() => {
    ...
    if (item !== null) {
      ...
      if (user) {
        setUserCode(
          code ? code[settings.lang] : problem.starterCode[settings.lang]
        );
      } else {
